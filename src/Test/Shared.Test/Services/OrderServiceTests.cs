...
public class OrderServiceTests
{
    ...
    [Fact]
    public async Task CreateOrderAsync_WithOrderItemsIncludingPerson_ShouldCreateOrderWithPersons()
    {
        // Arrange
        using var context = TestHelper.GetTestDbContext();
        var service = new OrderService(context, _mapper);
        ...
        var productType = TestDataFactory.CreateProductType(name: "Safety Equipment");
        var manufacturer = TestDataFactory.CreateManufacturer(name: "SafetyCo");
        var product = TestDataFactory.CreateProduct(productType.Id, manufacturer.Id, name: "Safety Vest");
        var variant = TestDataFactory.CreateVariant(product.Id, name: "Size M");
        var person1 = TestDataFactory.CreatePerson(firstName: "John", lastName: "Doe");
        var person2 = TestDataFactory.CreatePerson(firstName: "Jane", lastName: "Smith");
        ...
        // Create order with items, one with person and one without
        var orderItems = new List<CreateOrUpdateOrderItemModel>
        {
            TestDataFactory.CreateOrderItemModel(Guid.Empty, variant.Id, quantity: 2, personId: person1.Id),
            TestDataFactory.CreateOrderItemModel(Guid.Empty, variant.Id, quantity: 3, personId: person2.Id),
            TestDataFactory.CreateOrderItemModel(Guid.Empty, variant.Id, quantity: 1, personId: null)
        };
        ...
        var savedOrderItems = await context.OrderItems
            .Where(oi => oi.OrderId == result.Id)
            .ToListAsync();
        
        Assert.Equal(3, savedOrderItems.Count);
        Assert.Equal(2, savedOrderItems.Count(oi => oi.PersonId.HasValue));
        Assert.Single(savedOrderItems.Where(oi => !oi.PersonId.HasValue));
        
        var itemForPerson1 = savedOrderItems.FirstOrDefault(oi => oi.PersonId == person1.Id);
        Assert.NotNull(itemForPerson1);
        Assert.Equal(2, itemForPerson1.Quantity);
        
        var itemForPerson2 = savedOrderItems.FirstOrDefault(oi => oi.PersonId == person2.Id);
        Assert.NotNull(itemForPerson2);
        Assert.Equal(3, itemForPerson2.Quantity);
    }
}
