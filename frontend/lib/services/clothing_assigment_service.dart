...
class ClothingAssignmentService {
  ...
  Future<List<ClothingItemAssignmentHistoryModel>> getAll() async {
    ...
    return json
        .map((e) => ClothingItemAssignmentHistoryModel.fromJson(e))
        .toList();
  }

  Future<ClothingItemAssignmentHistoryModel> getById(String id) async {
    final response = await http.get('/assignments/$id');
    return ClothingItemAssignmentHistoryModel.fromJson(
      jsonDecode(response.body),
    );
  }

  Future<ClothingItemAssignmentHistoryModel> create(
    CreateClothingItemAssignmentHistoryModel model,
  ) async {
  ...
  Future<void> update(
    String id,
    ClothingItemAssignmentHistoryModel model,
  ) async {
